...
- Offline WoW: Forever quest database (`bridge/data/forever-quests.json`, about 4,400 quests) converted from the AllTheThings Forever data (MIT) by `npm run build:quests`, with ATT's notice kept in `bridge/data/README.md`. New guide tools `quests_near_me` (quests you can pick up on your map, nearest giver first, filtered by level, faction, race, class, completed/active quests and prerequisites) and `quest_info` (a quest's givers with coordinates and prerequisites). "What should I do?" questions get the nearby list in the prompt for every preset, so Gemini and local models can answer them too.
- Faster spoken replies: Fish Audio WebSocket streaming TTS (`fish.stream`), fed sentence by sentence while the OpenAI guide's answer is still streaming (`playerGuide.stream`), with PCM chunks played back to back in the companion as they arrive. A filler line ("Let me check that.") covers lookups that take longer than `fish.fillerDelayMs` (`fish.filler`). If the stream fails before any audio, the answer falls back to the one-shot WAV synthesis. The text answer reaches the game as soon as the model finishes, no longer after synthesis. Streaming and fillers can be switched off in companion Settings.
- Opt-in spoken announcements, each off by default and switchable in companion Settings or with `/wow-claude announce <quest|level|zone|bags|all> on|off`: quest objectives complete (and where to turn in), level up (with new trainer spells from `C_SpellBook.GetCurrentLevelSpells`), a one-sentence quest briefing on entering a new zone, bags nearly full or gear about to break. Never during combat, rate limited, and informational only.
- Quest narration: `/wow-claude narrate on` reads newly accepted quests aloud in the selected Fish voice, and "read me this quest" reads the selected quest on demand.
