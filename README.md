...
| Sourced game lookup | With an OpenAI key and preset, quest and other game-fact questions can trigger live web search. The answer includes cited URLs in game and clickable source buttons in the companion. The guide checks that cited pages match Forever and uses matching search results to corroborate broader Forever guides; it withholds routes supported only by unrelated or wrong-edition pages. |
