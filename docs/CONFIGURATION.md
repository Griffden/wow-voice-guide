...
All WoW-related questions default to **World of Warcraft: Forever** in both ordinary model answers and web lookup, unless the player explicitly asks about another edition. Saying “Classic or Forever” keeps the current Forever context. The guide prefers current in-game quest IDs, objectives, and linked tooltips and avoids importing Retail or Classic routes. It accepts a broader Forever guide citation when the same search found a matching quest or topic result, but withholds routes supported only by unrelated or wrong-edition citations. This is a heuristic, not proof that a page is correct for the new beta; unsupported steps should still be treated as unverified.
