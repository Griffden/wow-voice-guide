...
test('normalizes JSON, fenced JSON, percentages and bad waypoints', () => {
  ...
  assert.equal(Providers.normalizeAssistantResponse('{"speech":"Hi","basis":"game"}').basis, 'game');
...
test('buildMessages supplies game context, bounded history and the current question', () => {
  ...
  assert.match(messages[0].content, /say where it comes from instead of refusing/);
  assert.match(messages[0].content, /"basis":"game\|forever\|classic\|general"/);
  assert.match(messages[0].content, /earlier assistant replies as potentially mistaken/);
  const withNotes = Providers.buildMessages({ context: '', history: [], text: 'Where?', notes: 'Quest 1861 "Mirror Lake"' });
  assert.match(withNotes[0].content, /Wowhead Forever reference[^\n]*\nQuest 1861 "Mirror Lake"/);
});
...
test('OpenAI-compatible request sends the selected low-latency model and parses structured output', async () => {
  ...
  try {
    ...
    assert.deepEqual(answer, { display: 'Turn left.', speech: 'Take the next left.', basis: 'general', waypoint: null });
